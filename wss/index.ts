...
io.on("connection", (socket) => {
      ...
      .then(() => {
        ...
        socket.join(room, () => {
          if (
            ...
            // We need to pass `players`, when the room is full, so that the client can display the list of players
            io.to(room).emit("onReady", { state: true, players: io.nsps["/"].adapter?.rooms[room]?.sockets ?? {} });
          }
        });
      })
      .catch((err) => {
    ...
    apiService
      ...
      .then((result) => {
        io.to(result?.data.room).emit("randomNumber", {
          // FIXED: It was `string` originally. It should be `number` type.
  ...
  socket.on("leaveRoom", () => {
    apiService.getUserDetail(socket.id).then((result) => {
      io.to(result?.data.room).emit("onReady", { state: false, players: io.nsps["/"].adapter.rooms[result?.data.room]?.sockets ?? {}  });
  ...
  socket.on("disconnect", () => {
    apiService.getUserDetail(socket.id).then((result) => {
      socket.broadcast.to(result?.data.room).emit("onReady", { state: false, players: io.nsps["/"].adapter.rooms[result?.data.room]?.sockets ?? {} });
      apiService.removeUserFromRoom(socket.id).then(() => {
        socket.leave(result?.data.room);
      });
    });
